...
- `loadConfig`: Helper to load configuration.
- `loadConfigFromPath`: Helper to load configuration from a specific path.
...
- `VerificationStatus`: The status enum (FRESH, STALE_TIMESTAMP, STALE_SEMANTIC).
- `DriftingSource`: Information about a specific source file that is drifting.
